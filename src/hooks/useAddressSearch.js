  ...
  useEffect(() => {
    if (selectedState) {
      ...
      const citiesInState = [...new Set(
        allAddresses.current
          .filter(addr => addr.estado === selectedState)
          .map(addr => addr.cidade)
          .filter(Boolean)
      )].sort();
      setAvailableCities(citiesInState);

      // Limpar cidade selecionada se não estiver no estado atual
      if (selectedCity && !citiesInState.includes(selectedCity)) {
        setSelectedCity('');
      }
    } else {
      // Mostrar todas as cidades se nenhum estado estiver selecionado
